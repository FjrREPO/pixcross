...
export const useSupplyPool = () => {
  ...
  const stepTemplates: Step[] = [
    ...
    { step: 3, text: "Supplying to pool", status: "idle" },
  ...
  const mutation = useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        const pixcrossAddress = contractAddresses[chainId]
          ?.pixcross as HexAddress;
        ...
        const txHash = await writeContract(config, {
          ...
          abi: pixcrossAbi,
          functionName: "supply",
          args: [id, valueToBigInt(denormalizedAmount), userAddress],
          chainId,
        });
