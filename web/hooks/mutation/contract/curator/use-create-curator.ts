...
export const useCreateCurator = () => {
  ...
  const mutation = useMutation({
    mutationFn: async ({
      ...
      symbol: string;
      asset: HexAddress;
      pools: { id: string; poolId: string; allocation: number }[];
      ...
      try {
        ...
        const curatorAddress = contractAddresses[chainId].curatorFactory;

        const allocationsInUint256 = pools.map((pool) =>
          BigInt(pool.allocation),
        );
  ...
  return { steps, mutation, txHash, logs, loadingStates, currentStepIndex };
};
