...
import { pixcrossFaucetAbi } from "@/lib/abis/pixcross-faucet.abi";
...
export const useFaucetClaim = () => {
  ...
  const stepTemplates: Step[] = [
    ...
    { step: 3, text: "Claiming faucet", status: "idle" },
  ...
  const mutation = useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        const pixcrossFaucetAddress = contractAddresses[chainId].pixcrossFaucet;
        ...
        const allowance = await readContract(config, {
          ...
          args: [userAddress, pixcrossFaucetAddress],
          chainId,
        });
        ...
        if (Number(allowance) <= Number(denormalizedAmount)) {
          const approveTxHash = await writeContract(config, {
            ...
            args: [pixcrossFaucetAddress, valueToBigInt(denormalizedAmount)],
            chainId,
          });
        ...
        const txHash = await writeContract(config, {
          address: pixcrossFaucetAddress,
          abi: pixcrossFaucetAbi,
          functionName: "claimTokens",
          args: [tokenAddress, userAddress, valueToBigInt(denormalizedAmount)],
          chainId,
        });
